...
- Let's look at an example of student height. 
- Suppose we recorded the height of everyone in this class. 
- Let's say the average height is 5' 8''. 
...
- We could do so by comparing our sample's mean to the distribution of sample means that would be drawn from that population under repeated sampling (aka, a critical value under the Z (standard normal) distribution). 
...
- Based on our data, we can calculate a critical value  = $z_{\bar{x}} = \frac{\bar{x}-\mu}{\frac{\sigma}{\sqrt{N}}}$
...
These slides were partially adapted from:
